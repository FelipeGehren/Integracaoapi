...
def buscar_cep():
    ...
    while True:
        ...
        if status_code == 400:
            print("Erro. Verifique ese está digitando somente números ou se está digitando apenas 8 dígitos de CEP")
            continue
...
def exibir_cep(resposta):
    ...
    for chave, valor in resposta.items():
        if chave in ["logradouro, bairro", "localidade", "uf"]:
            print(f"{chave}: {valor}")
...
def editar_contato(lista_tipo_contato):
    ...
    if 0 <= codigo < len(lista_tipo_contato):
        ...
        endereco = input("Quer trocar de endereço (digite qualquer tecla ou deixe em branco para manter o mesmo.)")
